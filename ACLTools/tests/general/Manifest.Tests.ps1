Describe "Validating the module manifest" {
	...
	$manifest = (Test-ModuleManifest "$moduleRoot\ACLTools.psd1")
	[version]$moduleVersion = Get-Item "$moduleRoot\ACLTools.psm1" | Select-String -Pattern '\$script:PSModuleVersion = "(.*?)"' | ForEach-Object { $_.Matches[0].Groups[1].Value }
	Context "Basic resources validation" {
		It "Exports all functions in the public folder" {
			$files = Get-ChildItem "$moduleRoot\functions" -Recurse -File -Filter "*.ps1"
			$count = (Compare-Object -ReferenceObject $files.BaseName -DifferenceObject $manifest.ExportedFunctions.Keys).Count
			$count | Should be 0
		}
